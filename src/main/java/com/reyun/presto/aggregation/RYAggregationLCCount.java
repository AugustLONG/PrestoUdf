...
实例: 查询12月1号开始7天(最大15天), 往后推30天(最大30天)的日留存。
...
30,
xwhat,
'Action,payment') as temp
from orc_event_4f4825dde448c06fcdd15be294a7558b
where
(ds >= '2016-12-01' and ds < '2016-12-08' and xwhat = 'Action') or
(ds >= '2016-12-02' and ds < '2016-12-29' and xwhat = 'payment')
group by xwho);

实例: 查询12月5号开始3周(最大12周), 往后推2周(最大8周)的周留存。
...
from orc_event_4f4825dde448c06fcdd15be294a7558b
where
(ds >= '2016-12-05' and ds < '2016-12-26' and xwhat = 'Action') or
(ds >= '2016-12-12' and ds < '2016-12-29' and xwhat = 'payment')
group by xwho);
 */
@AggregationFunction("lc_count")
public class RYAggregationLCCount extends RYAggregationBase {
    ...
    public static void output(SliceState state, BlockBuilder out) {
        ...
        // 构造结果: 当前用户在第一个事件中每一天的状态, 和在第二个事件中每一天的状态
